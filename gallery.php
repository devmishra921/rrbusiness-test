...
<style>
:root{--brand:#a83232;--brand-light:#ff5e5e;--grad:linear-gradient(135deg,var(--brand),var(--brand-light));--brand-dark:#a83232}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:system-ui,Segoe UI,Roboto,Helvetica,Arial,sans-serif;background:#fff8f0;color:#333;line-height:1.5}
...
/* TITLE & GALLERY */
h1{text-align:center;margin:38px 0 16px;color:var(--brand);font-size:1.8rem}
.gallery{display:flex;flex-wrap:wrap;justify-content:center;gap:26px;padding:0 18px 60px}
.card{width:250px;background:#fff;border-radius:10px;box-shadow:0 2px 10px rgba(0,0,0,.09);transition:.3s;overflow:hidden;border:1px solid #eee}
.card:hover{transform:translateY(-4px);box-shadow:0 4px 16px rgba(0,0,0,.18)}
.img-wrap{position:relative}.img-wrap img{width:100%;height:180px;object-fit:cover}
.nav-arrow{position:absolute;top:50%;transform:translateY(-50%);background:rgba(0,0,0,.4);color:#fff;border:none;width:30px;height:44px;display:flex;align-items:center;justify-content:center;font-size:1rem;border-radius:4px;cursor:pointer;transition:.25s}
.nav-arrow:hover{background:rgba(0,0,0,.6)}.prev{left:6px}.next{right:6px}
.caption{padding:10px;text-align:center;font-weight:600;font-size:.93rem;background:#fafafa}
@media(max-width:576px){.card{width:92%}}

/* FOOTER */
footer{background:var(--grad);color:#f1f1f1;padding:40px 20px;margin-top:55px;font-size:.92rem}
.footer-container{display:flex;flex-wrap:wrap;justify-content:center;gap:38px;max-width:1200px;margin:auto}
.footer-col{flex:1 1 220px;min-width:200px;text-align:center}
.footer-col h4{color:#ffd772;margin-bottom:15px;font-size:1.02rem}
.footer-col ul{list-style:none}.footer-col li{margin-bottom:8px;display:flex;justify-content:center;gap:6px}
.footer-col a{color:#f1f1f1;text-decoration:none;transition:.25s}.footer-col a:hover{text-shadow:0 0 4px #fff}
.footer-bottom{text-align:center;margin-top:32px;font-size:.85rem}.footer-bottom p{color:#ffd772;margin:4px 0;font-size:.88rem}
</style>
</head>
<body>
<header>
...
/* your two upload folders */
$dir1 = 'uploads/gallery/';       // primary
$dir2 = 'admin/uploads/gallery/'; // fallback

$q = mysqli_query($conn, "SELECT image_path, caption FROM gallery_photos ORDER BY id DESC");
if ($q && mysqli_num_rows($q)) {
    while ($r = mysqli_fetch_assoc($q)) {
        // Break comma-list into an array
        $fns = array_filter(array_map('trim', explode(',', $r['image_path'])));
        if (!$fns) continue;

        // Build a list of existing file paths
        $paths = [];
        foreach ($fns as $f) {
            $p = $dir1 . $f;
            if (!file_exists($p) && file_exists($dir2 . $f)) {
                $p = $dir2 . $f;
            }
            if (file_exists($p)) {
                $paths[] = $p;
        ...
        // JSON-encode with HEX flags so quotes & apostrophes never break your attribute
        $payload    = json_encode($paths, JSON_HEX_QUOT | JSON_HEX_APOS);
        $firstImage = htmlspecialchars($paths[0], ENT_QUOTES);
        $cap        = htmlspecialchars($r['caption'], ENT_QUOTES);

        echo "
        <div class='card' data-imgs='{$payload}' data-idx='0'>
          <div class='img-wrap'>
            <button class='nav-arrow prev' onclick='navImg(this, -1)'>
              <i class='fa fa-chevron-left'></i>
            </button>
            <img src='{$firstImage}' alt='{$cap}'>
            <button class='nav-arrow next' onclick='navImg(this, 1)'>
    ...
    echo "<p style='text-align:center;color:#888;width:100%'>
...
<footer>
 <div class="footer-container">
  ...
  <div class="footer-col"><h4>🔗 Quick Links</h4><ul>
   <li><a href="dashboard.php">Dashboard</a></li><li><a href="admin_pannel.php">Manage Products</a></li>
   <li><a href="view_order.php">Orders</a></li><li><a href="customer_queries.php">Queries</a></li>
  </ul></div>
  ...
  <div class="footer-col"><h4>🛍️ Our Products</h4><ul>
   <li>✨ Haldi Powder</li><li>✨ Mirch Powder</li><li>✨ Garam Masala</li><li>✨ Chat Masala</li>
  </ul></div>
 </div>
...
/* burger */
document.getElementById('navToggle').onclick=()=>document.getElementById('navbar').classList.toggle('open');
/* slider */
function navImg(btn,step){
  const card=btn.closest('.card'),
        imgs=JSON.parse(card.dataset.imgs);
  let i=parseInt(card.dataset.idx);
  i=(i+step+imgs.length)%imgs.length;
  card.dataset.idx=i;
  card.querySelector('img').src=imgs[i];
}
</script>
<!-- Login/Signup Popup Modal -->
<div id="popupForm" class="modal">
